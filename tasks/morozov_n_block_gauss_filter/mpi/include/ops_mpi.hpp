...
class MorozovNBlockGaussFilterMPI : public BaseTask {
 ...
 private:
  ...
  static std::tuple<std::vector<uint8_t>, std::vector<int>> ParseImageToTiles(const std::vector<uint8_t> &src, int width,
  ...
  static void CopyTileWithBorders(const std::vector<uint8_t> &src, int width, int height, int x, int y, int t_width,
                                  int t_height, int l, int r, int u, int d, int w_over, int /* h_over */, int tile_start,
